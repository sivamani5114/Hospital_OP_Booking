...
import { Building2, Phone, Lock, ArrowLeft, ArrowRight, Stethoscope, CheckCircle2, KeyRound, ShieldCheck } from 'lucide-react';
  ...
  return (
    <div className="min-h-[calc(100vh-140px)] flex items-center justify-center p-3 sm:p-6 lg:p-10 w-full">
      <div className="w-full max-w-4xl glass-panel rounded-3xl border border-slate-800/90 shadow-2xl overflow-hidden flex flex-col md:flex-row items-stretch">
        ...
        {/* ═══ LEFT BRANDING PANEL (Emerald & Teal Medical SaaS) ═══ */}
        <div className="hidden md:flex flex-col justify-between w-[380px] flex-shrink-0 bg-gradient-to-br from-slate-900 via-emerald-950/40 to-slate-900 border-r border-slate-800 p-8 sm:p-10 relative overflow-hidden">
          <div className="absolute top-0 left-0 w-64 h-64 bg-emerald-500/10 rounded-full blur-3xl pointer-events-none -z-10"></div>
          <div className="absolute bottom-0 right-0 w-64 h-64 bg-teal-500/10 rounded-full blur-3xl pointer-events-none -z-10"></div>
          ...
          <div className="relative z-10">
            ...
            <h2 className="text-2xl sm:text-3xl font-extrabold text-white leading-tight mb-2.5 font-outfit">
              Manage OP Queues<br />
              <span className="text-transparent bg-clip-text bg-gradient-to-r from-emerald-400 to-teal-400">& Doctors Easily.</span>
            </h2>
            <p className="text-slate-300 text-xs leading-relaxed">
              Accept online OP bookings, configure doctor consultation slots, track daily collections, and issue digital prescriptions.
            </p>
          </div>
          ...
          <div className="space-y-3.5 my-6 relative z-10">
            {[
              { icon: '👨‍⚕️', title: 'Manage Doctors & Slots', desc: 'Add verified doctors with medical licenses' },
              { icon: '📋', title: 'Real-Time OP Queue', desc: 'Live token counter and queue caller' },
              { icon: '📄', title: 'Digital Prescriptions', desc: 'Attach consultation notes to patient records' },
          ...
          <div className="text-[11px] text-slate-400 border-t border-slate-800/80 pt-3 relative z-10 flex items-center gap-1.5">
            ...
            <span>CEA Compliant · Encrypted Desk Management</span>
          </div>
        </div>
        ...
        <div className="flex-1 p-6 sm:p-10 flex flex-col justify-between bg-slate-950/40">
          ...
          <div>
            ...
            <div className="mb-6 space-y-1.5">
              <div className="w-11 h-11 rounded-2xl bg-emerald-500/10 border border-emerald-500/30 flex items-center justify-center mb-3 text-emerald-400 shadow-md">
                <Building2 className="w-5 h-5" />
              </div>
              <h2 className="text-2xl font-extrabold text-white font-outfit">Hospital Desk Login</h2>
              <p className="text-xs text-slate-400">OP Consultation Desk & Reception Management Portal</p>
            </div>
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              <div>
                <div className="flex items-center justify-between mb-1.5">
                  <label className="text-xs text-slate-300 font-semibold">Hospital Official Phone (10 Digits)</label>
                ...
                <div className="relative flex items-center">
                  ...
                  <input
                    ...
                    inputMode="numeric"
                    maxLength={10}
                    placeholder="Enter Hospital 10-Digit Phone Number"
                    value={phone}
                    ...
                    className={`w-full bg-slate-900/90 border rounded-xl pl-20 pr-4 py-3 text-sm text-white font-mono tracking-wider outline-none transition-colors ${
                      phone.length === 10 && /^[6-9]\d{9}$/.test(phone)
                        ? 'border-emerald-500/80 focus:border-emerald-400 ring-2 ring-emerald-500/20'
                        : phone.length > 0 && !/^[6-9]/.test(phone)
              ...
              <div>
                <div className="flex items-center justify-between mb-1.5">
                  <label className="text-xs text-slate-300 font-semibold">Hospital Desk Password</label>
                  <button
                    type="button"
                ...
                <div className="relative">
                  ...
                  <input
                    type="password"
                    placeholder="Enter Hospital Desk Password"
                    value={password}
                    onChange={(e) => setPassword(e.target.value)}
                    className="w-full bg-slate-900/90 border border-slate-800 focus:border-emerald-500/60 rounded-xl pl-10 pr-4 py-3 text-sm text-white outline-none transition-colors"
                    required
                  />
                </div>
              </div>
              ...
              <button
                type="submit"
                className="w-full bg-gradient-to-r from-emerald-600 to-teal-600 hover:from-emerald-500 hover:to-teal-500 text-white font-bold py-3.5 rounded-xl text-sm shadow-lg shadow-emerald-500/25 flex items-center justify-center gap-2 active:scale-[0.98] transition-transform cursor-pointer mt-2"
              >
                Login to Hospital Control Portal <ArrowRight className="w-4 h-4" />
              </button>
            </form>
          </div>
          ...
          <div className="mt-6 pt-4 border-t border-slate-800/80">
            <div className="bg-emerald-950/40 p-3.5 rounded-2xl border border-emerald-500/30 flex flex-col sm:flex-row items-center justify-between gap-2.5 shadow-lg">
              <div className="text-left w-full sm:w-auto">
                <span className="text-xs text-emerald-300 font-bold flex items-center gap-1.5">
                  <KeyRound className="w-3.5 h-3.5 text-emerald-400" /> Hospital Desk Demo Access
                </span>
              ...
              <div className="flex items-center gap-1.5 w-full sm:w-auto">
                ...
                <button
                  ...
                  className="flex-1 sm:flex-none px-3.5 py-1.5 bg-gradient-to-r from-emerald-600 to-teal-600 hover:from-emerald-500 hover:to-teal-500 text-white rounded-xl text-xs font-bold shadow-md cursor-pointer transition-all active:scale-95 flex items-center justify-center gap-1"
